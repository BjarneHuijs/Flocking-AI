...
std::vector<Elite::Vector2> App_NavMeshGraph::FindPath(Elite::Vector2 startPos, Elite::Vector2 endPos)
{
	...
	const Triangle* start = m_pNavGraph->GetNavMeshPolygon()->GetTriangleFromPosition(startPos);
	const Triangle* end = m_pNavGraph->GetNavMeshPolygon()->GetTriangleFromPosition(endPos);
	...
	//Copy the graph
	auto graph = m_pNavGraph->Clone();
	//Create extra node for the Start Node (Agent's position)
	int startNodeIdx{ graph->GetNextFreeNodeIndex() };
	graph->AddNode(new NavGraphNode{ startNodeIdx, invalid_node_index, startPos });
	for (int lineIdx : start->metaData.IndexLines) 
	{
		for (auto node : graph->GetAllNodes()) 
		{
			if (node->GetLineIndex() == lineIdx) 
			{
				graph->AddConnection(new GraphConnection2D{ startNodeIdx, node->GetIndex(), Elite::Distance( graph->GetNode(startNodeIdx)->GetPosition(), node->GetPosition() ) });
			}
		}
	}
	//Create extra node for the End Node
	int endNodeIdx{ graph->GetNextFreeNodeIndex() };
	graph->AddNode(new NavGraphNode{ endNodeIdx, invalid_node_index, endPos });
	for (int lineIdx : end->metaData.IndexLines)
	{
		for (auto node : graph->GetAllNodes())
		{
			if (node->GetLineIndex() == lineIdx)
			{
				graph->AddConnection(new GraphConnection2D{ endNodeIdx, node->GetIndex(), Elite::Distance(graph->GetNode(endNodeIdx)->GetPosition(), node->GetPosition()) });
			}
		}
	}
	...
	auto path = pathFinder.FindPath(graph->GetNode(startNodeIdx), graph->GetNode(endNodeIdx));
	for (auto node : path) 
	{
